...
public class WebWordCounter {
    ...
    private static String TEST_URL = "http://textfiles.com/etext/FICTION/austen-emma-754.txt";
    ...
    public static void main(String [] args) {
        ...
        for(String aWord : wordList) {
            if(!isStopWord(aWord)) {
                if(wordCountMap.containsKey(aWord)) {
                    tempCount = wordCountMap.get(aWord);
                    wordCountMap.put(aWord, ++tempCount);
                }
                else {
                    wordCountMap.put(aWord, 1);
                }
            }
        }
        ...
        // appear greater then some frequency (for example, 5).

        System.out.println(wordCountMap.entrySet().stream()
